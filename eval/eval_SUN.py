"""
Evaluation script for TARDFormer_auto on SUNRGBD.
Usage:
    python eval_SUN.py --data-dir /path/to/SUNRGBD --ckpt /path/to/TARDFormer_auto.pth --visualize
"""
...
parser = argparse.ArgumentParser(description='SUNRGBD Evaluation')
parser.add_argument('--data-dir', required=True, metavar='DIR',
                    help='dataset root (expects train/test folders and split txt)')
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        depth = np.expand_dims(depth, 0).astype(float)
