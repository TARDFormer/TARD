"""
Evaluation script for TARDFormer_auto on NYUv2.
Usage:
    python eval.py --data-dir /path/to/NYUv2 --ckpt /path/to/TARDFormer_auto.pth --visualize
"""
...
parser = argparse.ArgumentParser(description='RGBD Semantic Segmentation Evaluation')
parser.add_argument('--data-dir', required=True, metavar='DIR',
                    help='dataset root (expects images/depths/labels and a split txt)')
parser.add_argument('--split', default='test.txt', help='split file name, e.g., test.txt')
...
parser.add_argument('--ckpt', required=True, type=str, metavar='PATH',
                    help='path to TARDFormer_auto checkpoint')
...
def build_model():
    # Single entry point to instantiate TARDFormer_auto
...
def inference():
    ...
    with torch.no_grad():
        # Warm-up a few iterations for stable timing
    ...
    print('Mean inference time per sample (ms):', timings.mean())
