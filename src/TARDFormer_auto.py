...
# **Progressive offset accumulation to generate deformable offsets**
class ProgressiveOffsetPredictor(nn.Module):
    def __init__(self, in_channels, hidden_dim=128):
        ...
        self.conv_mu = nn.Conv2d(hidden_dim, 18, kernel_size=1)       # predict μ
        self.conv_logvar = nn.Conv2d(hidden_dim, 18, kernel_size=1)   # predict log(σ²)
        self.conv_scale = nn.Conv2d(hidden_dim, 18, kernel_size=1)    # predict scaling σ
    ...
    def forward(self, x, prev_offset=None):
        ...
        offset = mu + sigma * eps  # sampled offset

        if prev_offset is not None:
            scale_factor = torch.sigmoid(self.conv_scale(x))  # σ_i in [0,1]
            offset = offset + scale_factor * prev_offset      # accumulate offset
...
class DeformASPP(nn.Module):
    def __init__(self, in_channels, out_channels, steps=3):
        ...
        self.deform_layers = nn.ModuleList([ 
            DeformConv2d(in_channels, out_channels, kernel_size=3, padding=1, bias=False) for _ in range(steps)
        ])
        ...
        self.project = nn.Sequential(
            nn.Conv2d(out_channels * (steps + 1), out_channels, kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        offset = None  # start with no previous offset

        for i in range(self.steps):
            offset, mu, sigma = self.bfn_layers[i](x, prev_offset=offset)  # predict offset
            offset_list.append(offset.detach())   # store offset
            mu_list.append(mu.detach())           # store mean
            sigma_list.append(sigma.detach())     # store std

            x = self.deform_layers[i](x, offset)  # deformable conv
            res.append(x)
...
# Replace DeepLabV3PlusDecoder with Deformable ASPP
class DeformableASPPDecoder(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, steps=3):
        super(DeformableASPPDecoder, self).__init__()
        self.aspp = DeformASPP(in_channels[-1], 256, steps=steps)
        ...
        self.mid_level_conv = nn.Sequential(
            nn.Conv2d(in_channels[1], 48, kernel_size=1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )
        
        self.high_level_conv = nn.Sequential(
            nn.Conv2d(in_channels[2], 48, kernel_size=1, bias=False),
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )
        
        self.final_conv = nn.Sequential(
            nn.Conv2d(256 + 48 * 3, 256, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
    ...
    def forward(self, inputs):
        ...
        # process c4 through ASPP
        aspp_out, _ , _, _= self.aspp(c4)  # ignore offset_list

        # process c1, c2, c3
        low_level_feat = self.low_level_conv(c1)
        mid_level_feat = self.mid_level_conv(c2)
        high_level_feat = self.high_level_conv(c3)
        
        # upsample ASPP output to c1 resolution
        aspp_out = F.interpolate(aspp_out, size=low_level_feat.size()[2:], mode='bilinear', align_corners=False)

        # upsample mid/high-level feats to c1 resolution
        mid_level_feat = F.interpolate(mid_level_feat, size=low_level_feat.size()[2:], mode='bilinear', align_corners=False)
        high_level_feat = F.interpolate(high_level_feat, size=low_level_feat.size()[2:], mode='bilinear', align_corners=False)
        
        # fuse all features
        fused_feat = torch.cat([aspp_out, low_level_feat, mid_level_feat, high_level_feat], dim=1)
        
        # final conv
        out = self.final_conv(fused_feat)
        return out

class TARDFormer_auto(nn.Module):
    def __init__(self, num_classes):
        super(TARDFormer_auto, self).__init__()
        ...
        self.Decoder = DeformableASPPDecoder(in_channels=self.channel, low_level_channels=self.channel[0], num_classes=num_classes)
...
if __name__ == '__main__':
    model = TARDFormer_auto(num_classes=40)
    model.eval()
